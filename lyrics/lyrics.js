  ...
  function getSongs(object) {
    if (object.message.header.status_code === 404) {
      ...
      object.message.body.track_list.forEach(element => {
        document.querySelector(".results").innerHTML += '<tr><th scope="row">'+(i+1)+'</th><td><b>'+element.track.track_name+'></b><br>'+element.track.artist_name+'</td><td><a target="_blank" href="'+element.track.track_share_url.substring(0,(element.track.track_share_url.length - 75))+'" class="btn btn-danger">Lyrics</a></td></tr>';        
        i++;
      });
      i =0;
